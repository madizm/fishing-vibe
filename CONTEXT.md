...
- **收录 (spot intake)** — the pipeline that turns a Douyin video URL or search keyword into persisted 钓点: extract page text/comments → extract place candidates and 鱼种 → geocode → score quality → write to SQLite. Implemented by the `spot_intake` package.
