...
| 10 | 语音识别视频内容 | 下载/提取视频音频并进行 ASR，补充标题/文案无法覆盖的钓点、鱼种、钓法信息 | 待优化 |
...
当前首版主要依赖搜索结果、视频标题、页面文案、章节摘要和评论文本识别钓点与鱼种。后续可增加视频语音识别能力，将视频中的口播内容转成文本后参与抽取。

优化目标：

- 对视频音频进行 ASR 识别，得到字幕/转写文本
- 从转写文本中补充识别钓点、鱼种、钓法、饵料、天气、水情等信息
- 与标题、文案、章节摘要、评论结果合并去重
- 为语音来源字段增加标记，例如 `fish_species_source=asr`、`place_source=asr`
- 对 ASR 抽取结果设置较低初始置信度，并结合多来源命中提升可信度

建议流程：

```text
1. 获取视频详情页中的视频播放地址或可下载资源
2. 提取音频轨道
3. 使用本地 Whisper / faster-whisper / 云端 ASR 转写语音
4. 将 ASR 文本输入现有地名识别、鱼种识别流程
5. 将 ASR 文本片段保存到 source_text，便于回溯
```

注意事项：

- 音视频下载需遵守平台规则和版权要求
- ASR 批量处理成本较高，应优先对高价值/高热度视频执行
- 口播中常出现模糊地名，需要继续结合城市前缀、POI、评论和地理编码分数校验
- 方言、背景噪声、音乐可能降低识别准确率，应保留原始转写文本供人工核验
